...
from src.database import TokenManager, DBUserTable, async_session
from src.database.schemas import SCreateUser, SCredentials
import hashlib


token_manager = TokenManager()
db_user_table = DBUserTable(async_session=async_session)
router = APIRouter(
    prefix="/auth",
    tags=["auth"]
)
...
async def registration(credentials: SCredentials, request: Request, response: Response):
    ...
    db_user = await db_user_table.get_by_username(username=username)
    if db_user:
    ...
    created_uid = await db_user_table.create_one(user=user_create)
    if not created_uid:
        return "Something went wrong"
    
    new_tokens = await token_manager.get_pair_tokens(id=created_uid)
    result = db_user_table.update_refresh_token(
        uid=created_uid,
...
async def login(credentials: SCredentials, request: Request, response: Response):
    ...
    await db_user_table.update_refresh_token(
        uid=db_user.uid,
        new_refresh_token=new_tokens.get("jwt_refresh_token")
    )
    return f"Hello, {username}"